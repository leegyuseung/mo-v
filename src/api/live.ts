 ...
 */
export async function fetchLiveStreamers(liveOnly = false): Promise<LiveStreamer[]> {
  const query = liveOnly ? "?liveOnly=1" : "";
  const response = await fetch(`/api/live/all${query}`);
  if (!response.ok) {
