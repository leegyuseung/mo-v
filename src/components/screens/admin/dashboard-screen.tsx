...
import type { SectionTitleProps } from "@/types/admin";
...
export default function DashboardScreen() {
  ...
  const entityBase = stats
    ? Math.max(stats.totalStreamers, stats.totalGroups, stats.totalCrews, 1)
    : 1;
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-10">
      ...
      <section>
        <SectionTitle
          title="등록 자산"
          description="서비스에 등록된 버츄얼, 그룹, 소속 수입니다."
        />
        {isLoading ? (
      ...
      <section>
        <SectionTitle
          title="요청 대기"
          description="관리자 처리 대상인 등록 대기, 정보 수정 요청, 신고 요청 수입니다."
        />
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
            {[...Array(3)].map((_, index) => (
