...
import { toggleNoticeLike } from "@/api/notice";

type NoticeDetailLikeButtonProps = {
  noticeId: number;
...
/**
 * 공지사항 상세 좋아요 버튼.
 * 왜: 서버에서 전달받은 초기 카운트를 유지하면서 상호작용만 클라이언트에서 처리하기 위함.
 */
export default function NoticeDetailLikeButton({
  noticeId,
  initialLikeCount,
  initialLiked,
}: NoticeDetailLikeButtonProps) {
  ...
  async function handleClick() {
    ...
    try {
      const result = await toggleNoticeLike(noticeId);
