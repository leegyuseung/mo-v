...
type CommunityCrewDetailPageProps = {
  params: Promise<{
    crewCode: string;
  }>;
...
export async function generateMetadata({
  params,
}: CommunityCrewDetailPageProps): Promise<Metadata> {
  const { crewCode } = await params;
  const meta = await fetchCommunityCrewBoardMetaOnServer(crewCode);
  const crewName = meta?.entityName || "소속 커뮤니티";

  return {
    title: crewName,
    description: `${crewName}의 커뮤니티 게시글을 확인하세요.`,
    alternates: {
      canonical: `/community/crew/${crewCode}`,
    },
  };
}

export default async function CommunityCrewDetailPage({
  params,
  searchParams,
}: CommunityCrewDetailPageProps) {
  const { crewCode } = await params;
