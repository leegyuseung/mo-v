...
type CommunityCrewPostDetailPageProps = {
  params: Promise<{
    crewCode: string;
    postId: string;
  }>;
...
export async function generateMetadata({
  params,
}: CommunityCrewPostDetailPageProps): Promise<Metadata> {
  const { crewCode, postId } = await params;
  ...
  return {
    ...
    alternates: {
      canonical: `/community/crew/${crewCode}/${parsedPostId}`,
    },
  };
}

export default async function CommunityCrewPostDetailPage({
  params,
}: CommunityCrewPostDetailPageProps) {
  const { crewCode, postId } = await params;
  const parsedPostId = Number(postId);
