...
export async function generateMetadata({
  params,
}: CommunityGroupDetailPageProps): Promise<Metadata> {
  const { groupCode } = await params;
  const meta = await fetchCommunityGroupBoardMetaOnServer(groupCode);
  const groupName = meta?.entityName || "그룹 커뮤니티";

  return {
    title: groupName,
    description: `${groupName}의 커뮤니티 게시글을 확인하세요.`,
    alternates: {
      canonical: `/community/group/${groupCode}`,
    },
  };
}

export default async function CommunityGroupDetailPage({
  params,
  searchParams,
}: CommunityGroupDetailPageProps) {
  const { groupCode } = await params;
